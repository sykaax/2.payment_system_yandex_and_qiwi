<?php
// на этой странице происходит авторизация эмаил админ
...
//делаем запрос в базу данных на наличие такой почты и отправляем на почту письмо
  ...
  $mysqlemailauth = mysqli_query($dbconnect, "SELECT email FROM `tabadmins` WHERE `email` = '$emailauth' ");
	...
	$sqlhashforemail = mysqli_query($dbconnect, "UPDATE `tabadmins` SET `hashforemailadmin` = '$hashforemail' 
	...
	$mail = mail("$emailauth", "Мои продажи $linkmyglobal", "Добрый день! Продажи вашего магазина вы можете посмотреть здесь: $linkmyglobalhttpadmin/authmailadmin.php?hashforemail=$hashforemail");
 ...
 elseif (isset($_GET['hashforemail']) and !isset($_SESSION['hashsessionadmin'])){ //проверка пришёл человек по ссылке и есть ли у него GEt параметр
  ...
  $hashforemail = $_GET['hashforemail']; // хеш который будем проверять
   ...
   $hashforemail = funcformatstremail($hashforemail);
	...
	$mysqlhashforemalget = mysqli_query($dbconnect, "SELECT * FROM `tabadmins` WHERE 
	`hashforemailadmin` = '$hashforemail'"); // запрос есть ли эмаил с такм хешем или нету
	...
	$sqlhashsession = mysqli_query($dbconnect, "UPDATE `tabadmins` SET 
	`hashsessionadmin` = '$hashsession' , `hashforemailadmin` = '' WHERE 
	`hashforemailadmin` = '$hashforemail' ");   // обновление таблицы и добавления генерированного хеша, удаление 
	// активированного get хеша из письма
	
	$_SESSION['emailadmin'] = $authemails;
	$_SESSION['hashsessionadmin'] = $hashsession;
	
	echo '<script type="text/javascript">location="mycabadmin.php";</script>';
			
   }
   else{ // если человек дал неверный get параметр выйдет ошибка
				echo $hashforemail;
				echo " <div class='form'><h3>Ваша ссылка недействительна</h3><br/>Нажмите тут чтобы сгеннерировать <a href='authmailadmin.php'>Новая ссылка</a></div>";
				}
    }
elseif( isset($_SESSION['hashsessionadmin'])){
...
<div class="payment-types-title"><img  class="logopay" src="../images/logopay.png"  width="50">Вход в кабинет администратора<br><div class="linkpayment">payment.comx - приём платежей</div></div>
<ul id="menudesk">
...
Введите ваш E-mail и капчу, после чего вам на почту поступит авторизационная ссылка.
</div>
